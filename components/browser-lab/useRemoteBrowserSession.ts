...
export function useRemoteBrowserSession({
  ...
  const handleLaunch = useCallback(async () => {
    ...
    if (!browserbaseConfigured) {
      setStatus("error");
      setError(
        "The remote browser is not connected in this environment. Use the labelled fallback preview below.",
      );
      window.setTimeout(() => errorRef.current?.focus(), 0);
      return;
    }
    ...
    try {
      ...
      const message =
        launchError instanceof DOMException && launchError.name === "AbortError"
          ? "The browser launch request timed out. No live view was returned; use the fallback preview or try again."
